...
Тексты обеих "страниц" установки (приветствие и результат гео-проверки) берутся из `install.config.json` рядом со скриптом — правьте его напрямую, `install.ps1` менять не нужно:

```json
{
  "welcome": "Текст приветствия (страница 1)",
  "secondPage": {
    "ua": "Текст для Украины",
    "other": "Текст для остальных стран, {0} — подставится код страны"
  }
}
```

Если файла нет или в нём не хватает ключа — используется значение по умолчанию из самого скрипта.
